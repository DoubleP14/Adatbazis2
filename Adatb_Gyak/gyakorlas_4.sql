...
CREATE OR REPLACE PROCEDURE pr_brigad_hozzarendel(
    ...
    v_atfedés_szam NUMBER;
    
    crew_booked_exc EXCEPTION;
    PRAGMA EXCEPTION_INIT(crew_booked_exc, -20003);
BEGIN
    -- Átfedés ellenőrzése
    -- Akkor van átfedés, ha (LétezőKezdet <= ÚjVég) ÉS (LétezőVég >= ÚjKezdet)
    SELECT COUNT(*)
    INTO v_atfedés_szam
    FROM projekt_brigad
    ...
    ELSE
        ...
        -- COMMIT; -- Opcionális, de eljárásokban szokás lehet, ha nem tesztelünk rollback-el
    END IF;

EXCEPTION
    WHEN crew_booked_exc THEN
        DBMS_OUTPUT.PUT_LINE('Hiba: A brigád ebben az időszakban már foglalt!');
    WHEN OTHERS THEN
